...
const fetchItunesImage = (track, cb) => {
  ...
  const url = `https://itunes.apple.com/search?media=music&term=${trackDesc.replace(
    / /g,
    '+'
  )}`;
...
exports.handler = async (event) => {
  ...
  try {
    const response = await new Promise((resolve, reject) =>
      async.whilst(testRepeating, requestTracks, (err) => {
        if (err) {
          log(err);
          reject(err);
          return;
        ...
        const returnWithImage = (imageErr, base64, imageSource) => {
          log(
            `Fetched ${imageSource} image: ${
              base64 ? 'success' : imageErr.message
            }`
          );
        ...
        if (uri) {
          ...
          fetchBase64Image(uri, (imageErr, base64) =>
            returnWithImage(imageErr, base64, 'lastfm')
          );
        } else {
          fetchItunesImage(mergedTrack, (imageErr, base64) =>
            returnWithImage(imageErr, base64, 'itunes')
          );
        }
      })
    );
